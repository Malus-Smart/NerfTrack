...
const messages = {
  ...
  'setup.description.desktop': [
    ...
    '連接本機 Codex 桌面資料，根據權杖用量估算每週 API 等值。',
  ],
  ...
  'settings.localPrices': ['Token prices stay local', '令牌价格保留在本地', '權杖價格保留在本機'],
  'settings.pricingDescription': [
    ...
    '覆盖价格仅保存在本地，并优先于 models.dev 和 NerfTrack 内置的备用价格。可用于尚未定价的模型或本地别名；价格单位为每百万令牌的美元金额。',
    '覆寫價格僅儲存在本機，並優先於 models.dev 和 NerfTrack 內建的備用價格。可用於尚未定價的模型或本機別名；價格單位為每百萬權杖的美元金額。',
  ],
  ...
  'diagnostics.pricedEvents': ['Priced token events', '已定价令牌事件', '已定價權杖事件'],
  ...
  'history.observedCost': ['Observed token cost', '已观测令牌成本', '已觀測權杖成本'],
  ...
  'history.pendingDescription': [
    ...
    '在令牌成本正增量与每周用量正增量配对前，它们不会出现在图表中。',
    '在權杖成本正增量與每週用量正增量配對前，它們不會出現在圖表中。',
  ],
  ...
  'home.observedTokenCost': ['Observed Token Cost', '已观测令牌成本', '已觀測權杖成本'],
